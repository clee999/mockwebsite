 function equivalentCheck(){ // This allows the last value to remain in view until other button are pressed;
 ...
 function input(x){
   ...
   if(document.getElementById('decimalVar').value == 0){
      ...
      document.getElementById('result').value = x; //Result x to the text output
      ...
      if(decimalCount == 1){
         ...
         x *= 1/10; //We use math to place the decimal point 
         y += x;
   ...
   /*if (document.getElementById('result').value == 0) //This checks the zero and get rid of it for the first time. 
      {
         document.getElementById('result').value = x;
      } 
   else {
